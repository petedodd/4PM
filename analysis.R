...
getHAORs <- function(x,y){
    ...
    z <- z %*% hivartOR$U      #make these correlated normals - check transpose
...
BURD <- function(x){
    ans <- QG(x,shape=BB$k,scale=BB$S) #NB for length(x)=nrow(BB) !
...
BURDH <- function(x) {
    ans <- QG(x,shape=BB$hk,scale=BB$hS) #NB for length(x)=nrow(BB) !
...
BURDA <- function(x) {
    ## warns when ak==Inf
    ans <- QG(x,shape=BB$ak,scale=BB$aS) #NB for length(x)=nrow(BB) !
    ans[is.na(ans)] <- 0
    ans[ans>1] <- .999
    ans
}

test <- BURDA(runif(nrow(BB)))
...
## no chnage...
BB$notifs[is.na(BB$notifs)] <- 0
BB$notifsY[is.na(BB$notifsY)] <- 0
BB$notifsO[is.na(BB$notifsO)] <- 0
...
txstat <- c('On tx (HIV-)','Off tx (HIV-)','On tx (HIV+)','Off tx (HIV+)')
...
for(i in 1:length(senscns))
    senswho[i] <- which(as.character(BB$iso3)==senscns[i])
F <- rep(1,nrow(BB))                    #mult factor for notifications
if(sensNote)
    F[senswho] <- 1/(1-senspriv)            #mult inflation factor

## n replicates
nrep <- 1e3*10
RESA <- matrix(NA,nrow = nrow(BB)*nrep,ncol=12) #for results
RESA[,1] <- 1:nrow(BB)
...
## BB contains the country data - calx done world-block-wise 
for(i in 1:nrep){
    if(!i%%1e3) print(i)
    now <- (i-1)*nrow(BB) + 1:nrow(BB)  # replicate block for all countries
    ## get parameters
    ...
    young <- vrbeta(BB[,c('aa','ab')])  #proportion young
    ## deaths
    ## --young
    DeathsOnTxY <- F*BB$notifsY * cfrz1y * (1-tbhiv) #F is notification inflation =1 unless priv
    DeathsOffTxY <- pmax(burden * young-F*BB$notifsY,0) * cfrz0y * (1-tbhiv)
    DeathsOnTxHY <- F*BB$notifsY * (cfrzh1aY * tbart + cfrzh1Y * (1-tbart)) * tbhiv 
    DeathsOffTxHY <- pmax(burden * young-F*BB$notifsY,0) * tbhiv * (cfrzh0aY * tbart + cfrzh0Y * (1-tbart)) #averages CFR
    ## --old
    DeathsOnTxO <- F*BB$notifsO * cfrz1o * (1-tbhiv)
    DeathsOffTxO <- pmax(burden * (1-young)-F*BB$notifsO,0) * cfrz0o * (1-tbhiv)
    DeathsOnTxHO <- F*BB$notifsO * (cfrzh1aO * tbart + cfrzh1O * (1-tbart)) * tbhiv
    DeathsOffTxHO <- pmax(burden * (1-young)-F*BB$notifsO,0) * tbhiv * (cfrzh0aO * tbart + cfrzh0O * (1-tbart)) #averages CFR
    ## record results
...
## neaten
colnames(RESA) <- c('id','DeathsOnTx','DeathsOffTx','DeathsOnTxH','DeathsOffTxH',
                    'DeathsOnTxY','DeathsOffTxY','DeathsOnTxHY','DeathsOffTxHY',
                    'HIV','HIVinTB','HIVinDeaths')
RESA <- as.data.frame(RESA)
RESA$iso3 <- BB$iso3
RESA$country <- BB$country
RESA$g_whoregion<- BB$g_whoregion
RESA$replicate <- rep(1:nrep,each=nrow(BB))
...
RESATOT <- RESA[,list(total = sum(DeathsOnTx,na.rm=TRUE)+sum(DeathsOffTx,na.rm=TRUE)+
                        sum(DeathsOnTxH,na.rm=TRUE)+sum(DeathsOffTxH,na.rm=TRUE)),
...
RESAGs <- RESAG[,list(OnTx=quantile(OnTx,.5),OffTx=quantile(OffTx,.5),
                    OnTxH=quantile(OnTxH,.5),OffTxH=quantile(OffTxH,.5),
                    OnTx0=quantile(OnTx,.025),OffTx0=quantile(OffTx,.025),
                    OnTxH0=quantile(OnTxH,.025),OffTxH0=quantile(OffTxH,.025),
                    OnTx1=quantile(OnTx,.975),OffTx1=quantile(OffTx,.975),
                    OnTxH1=quantile(OnTxH,.975),OffTxH1=quantile(OffTxH,.975)
                    ),
            by=list(g_whoregion)]
...
RESATs <- RESAT[,list(OnTx=quantile(OnTx,.5),OffTx=quantile(OffTx,.5),
                   OnTxH=quantile(OnTxH,.5),OffTxH=quantile(OffTxH,.5),
                   OnTx0=quantile(OnTx,.025),OffTx0=quantile(OffTx,.025),
                   OnTxH0=quantile(OnTxH,.025),OffTxH0=quantile(OffTxH,.025),
                   OnTx1=quantile(OnTx,.975),OffTx1=quantile(OffTx,.975),
...
RESAGnc <- RESA[,list(total=sum(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH)),
            by=list(replicate,g_whoregion)]

RESAGncs <- RESAGnc[,list(total=median(total),total0=quantile(total,.025),
                        total1=quantile(total,.975)),
                  by=list(g_whoregion)]

RESAGnca <- RESA[,list(total=sum(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH)),
            by=list(replicate)]
...
MID <- MID[,-1]
HI <- HI[,-1]
MID <- cbind(MID,RESAGncs[order(g_whoregion),total])
LO <- cbind(LO,RESAGncs[order(g_whoregion),total0])
HI <- cbind(HI,RESAGncs[order(g_whoregion),total1])
gm <- c(unlist(RESATs[,1:4,with=FALSE]),median(RESAGnca$total))
gl <- c(unlist(RESATs[,5:8,with=FALSE]),quantile(RESAGnca$total,.025))
gh <- c(unlist(RESATs[,9:12,with=FALSE]),quantile(RESAGnca$total,.975))
MID <- rbind(MID,gm)
LO <- rbind(LO,gl)
HI <- rbind(HI,gh)
...
RESAGsy <- RESAGy[,list(OnTx=quantile(OnTx,.5),OffTx=quantile(OffTx,.5),
                    OnTxH=quantile(OnTxH,.5),OffTxH=quantile(OffTxH,.5),
                    OnTx0=quantile(OnTx,.025),OffTx0=quantile(OffTx,.025),
                    OnTxH0=quantile(OnTxH,.025),OffTxH0=quantile(OffTxH,.025),
                    OnTx1=quantile(OnTx,.975),OffTx1=quantile(OffTx,.975),
...
RESAGncy <- RESA[,list(total=sum(DeathsOnTxY+DeathsOffTxY+DeathsOnTxHY+DeathsOffTxHY)),
            by=list(replicate,g_whoregion)]

RESAGncsy <- RESAGncy[,list(total=median(total),total0=quantile(total,.025),
                        total1=quantile(total,.975)),
                  by=list(g_whoregion)]

RESAGncay <- RESA[,list(total=sum(DeathsOnTxY+DeathsOffTxY+DeathsOnTxHY+DeathsOffTxHY)),
            by=list(replicate)]
...
RESATB <- data.frame(Treatment=rep(c('On','Off'),each=2),Age=c('0-4','5-14'),Number=unlist(RESATB[1,]))
...
mosdat <- data.frame(cat = names(RESATBM),
                     ...
                     tbtx = rep(c('Treated for TB','Treated for TB',
                                  'Untreated for TB','Untreated for TB'),2),
...
RESAc <- RESA[,list(age=mid(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH),
                    aget=uq(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH),
                    ageb=lq(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH)),
              by=iso3]
...
RESAC <- RESA[,list(deaths=midr(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH),
                    deathslo=lbr(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH),
                    deathshi=ubr(DeathsOnTx+DeathsOffTx+DeathsOnTxH+DeathsOffTxH),
                    deathsY=midr(DeathsOnTxY+DeathsOffTxY+DeathsOnTxHY+DeathsOffTxHY),
                    deathsYlo=lbr(DeathsOnTxY+DeathsOffTxY+DeathsOnTxHY+DeathsOffTxHY),
                    deathsYhi=ubr(DeathsOnTxY+DeathsOffTxY+DeathsOnTxHY+DeathsOffTxHY),
                    ...
                    HIV = midr(1e3*HIV),HIVinTB = midr(1e3*HIVinTB),HIVinDeaths = midr(1e3*HIVinDeaths),
                    HIVlo = lbr(1e3*HIV),HIVinTBlo = lbr(1e3*HIVinTB),HIVinDeathslo = lbr(1e3*HIVinDeaths),
                    HIVhi = ubr(1e3*HIV),HIVinTBhi = ubr(1e3*HIVinTB),HIVinDeathshi = ubr(1e3*HIVinDeaths)
                    ),
